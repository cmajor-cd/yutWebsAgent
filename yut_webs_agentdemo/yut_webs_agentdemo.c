...
#include    "goahead.h"
#include    "js.h"
#include "../libs/cJSON/cJSON.h"  //yutWebsAgentDemo
...
/*
    Implement /action/actionTest. Parse the form variables and echo back.
    * yangyutong:2019
    * actionWebsAgentDemoEntry:官方只实现了form格式数据的解析,此处实现ajax.
    ...
    * echo result JSON格式：
    ...
    https://blog.csdn.net/weixin_30415113/article/details/95998145
    http://www.doc88.com/p-9915638640912.html
    wp->input

    ssize websGetContentLen(Webs *wp){
        return bufLen(&(wp->input));
    }
    ssize websPeekContent(Webs *wp, char* buff, int buff_len){
        int len = bufGetBlk(&(wp->input), buff, buff_len);
        buff[len] = '\0'; //插入结束符
        return len;
    }
 */
...
#define SUBMIT_NAME_VALUE "submitNameValue"
#define GET_NET_CFG "getNetworkCfgInfor"
...
static void actionWebsAgentDemoEntry(Webs *wp)
{
    ...
    if(!p_json_root)
    {
        ...
        // it's a normal form request, route to goAction API to process.
        printf("Log[route to goAction API to process]\n");
        SUBMIT_NAME_VALUE_func(wp);
        return;
    }
    else
    {
        ...
        if(!strcasecmp(p_json_node_value, GET_NET_CFG)){
            GET_NET_CFG_func(p_json_root, wp);
        }
    }
}
/**
* 此例为使用cJson库来解析ajax消息来获得参数value。
* input JSON格式：
    { "command: "xxCommand",
      "dat":{"key1":"xxValue"}
    }
...
int GET_NET_CFG_func(cJSON* json_node, Webs *wp){
    ...
    cJSON* _json_node = NULL;
    char* _json_node_value;
    _json_node = cJSON_GetObjectItem(json_node, JSON_COMMAND);
    _json_node_value = cJSON_GetStringValue(_json_node);
...
/**
 * 此例为goAction缺省实现接收和解析form报文数据。
 * 直接使用goAction的API函数 websGetVar() 来获得参数value。
*/
int SUBMIT_NAME_VALUE_func(Webs *wp){
	cchar *name, *address;
